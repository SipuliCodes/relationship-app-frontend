...
interface ClaimsObject {
  [index: number]: [string, string | number, string];
}
...
const populateClaim = (
  ...
  const claimsArray: [string, string | number, string] = [
    claim,
    value,
    description,
  ];
...
const changeDateFormat = (date: string | number): string => {
  const dateObj = new Date(Number(date) * 1000);
  return `${date} - [${dateObj.toString()}]`;
};

export const createClaimsTable = (claims: Claim): ClaimsObject => {
...
export const compareIssuingPolicy = (
  ...
  const tfpMatches =
    hasProperty(idTokenClaims, "tfp") &&
    idTokenClaims["tfp"].toString().toLowerCase() ===
      policyToCompare.toLowerCase();
  const acrMatches =
    hasProperty(idTokenClaims, "acr") &&
    idTokenClaims["acr"].toString().toLowerCase() ===
      policyToCompare.toLowerCase();
